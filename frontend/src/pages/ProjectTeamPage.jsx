...
export const ProjectTeamPage = () => {
  ...
  if (loading) return <div className="h-64 rounded-3xl bg-slate-100 animate-pulse" />;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Project: <span className="font-bold text-slate-700">{project.title}</span> • {project.members?.length || 1} / {project.teamSize} Formed
          </p>
        </div>
      ...
      {/* Member Roster Cards */}
      <div>
        <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500 mb-4">Current Team Roster</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(project.members || []).map((m, idx) => {
            ...
            return (
              <div key={idx} className="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-soft">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <h4 className="font-bold text-slate-900 text-sm truncate">{memberUser.name}</h4>
                      <Badge variant="brand" size="sm">{m.role || 'Member'}</Badge>
                    </div>
                    <p className="text-xs text-slate-500 truncate">{memberUser.headline}</p>
                    <p className="text-[11px] text-slate-400 mt-0.5 truncate">{memberUser.college}</p>
                  </div>
                </div>

                {/* Skills chips */}
                <div className="mb-4">
                  <span className="text-[11px] font-bold text-slate-400 block mb-1">Contributed Skills:</span>
                  <div className="flex flex-wrap gap-1">
                    {(memberUser.skills || []).map((s, sIdx) => (
                      <span key={sIdx} className="text-[11px] font-medium px-2 py-0.5 bg-slate-50 text-slate-700 rounded border border-slate-200">
                        {s.name}
                      </span>
                    ))}
                  </div>
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
                  ...
                  <Link to={`/profile?id=${memberUser._id}`} className="font-semibold text-brand-600 hover:underline">
                    View Profile →
                  </Link>
                </div>
              </div>
