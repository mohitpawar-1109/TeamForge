...
export const ProjectTasksPage = () => {
  ...
  const columns = [
    { id: 'TODO', title: 'TODO', badgeColor: 'bg-slate-100 text-slate-700' },
    { id: 'IN PROGRESS', title: 'IN PROGRESS', badgeColor: 'bg-amber-50 text-amber-700 border-amber-200' },
    { id: 'DONE', title: 'DONE', badgeColor: 'bg-emerald-50 text-emerald-700 border-emerald-200' }
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-soft">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100">
          <div>
            <Link to={`/projects/${id}`} className="inline-flex items-center gap-1 text-xs font-semibold text-brand-600 hover:underline mb-2">
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Collaborative Kanban board for <span className="font-bold text-slate-700">{project?.title}</span>
            </p>
          </div>
        ...
        <div className="pt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4 text-xs">
            <span className="font-bold text-slate-700">{stats.total} Total Tasks</span>
            <span>•</span>
            <span className="text-emerald-600 font-bold">{stats.completed} Completed</span>
            <span>•</span>
            <span className="text-amber-600 font-bold">{stats.inProgress} In Progress</span>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-xs font-extrabold text-slate-800">{stats.progressPercent}% Overall Progress</span>
            <div className="w-36 bg-slate-100 h-2.5 rounded-full overflow-hidden border border-slate-200">
              <div
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {columns.map((col) => {
          ...
          return (
            <div key={col.id} className="bg-slate-50 rounded-2xl border border-slate-200/80 p-4 flex flex-col min-h-[500px]">
              {/* Column Header */}
              ...
              <div className="space-y-3 flex-1 overflow-y-auto">
                {colTasks.length === 0 ? (
                  ...
                  colTasks.map((task) => (
                    <div
                      key={task._id}
                      className="bg-white rounded-xl border border-slate-200 p-4 shadow-xs hover:shadow-md transition-all group"
                    >
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleOpenEditModal(task)} className="p-1 text-slate-400 hover:text-slate-700">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => handleDeleteTask(task._id)} className="p-1 text-slate-400 hover:text-rose-600">
                            <Trash2 className="w-3.5 h-3.5" />
                          </button>
                        </div>
                      </div>

                      <h4 className="font-bold text-slate-900 text-sm mb-1.5 leading-snug">{task.title}</h4>
                      {task.description && (
                      ...
                      <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                        ...
                        <select
                          ...
                          className="text-[11px] bg-slate-50 border border-slate-200 rounded px-1.5 py-0.5 text-slate-700 font-medium focus:outline-none"
                        >
